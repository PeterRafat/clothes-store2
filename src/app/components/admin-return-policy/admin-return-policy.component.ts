...
@Component({
  selector: 'app-admin-return-policy',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './admin-return-policy.component.html',
  styleUrls: ['./admin-return-policy.component.css']
})
export class AdminReturnPolicyComponent implements OnInit {
  returnPolicyItems: ReturnPolicyItem[] = [];
  loading = false;
  editingItem: ReturnPolicyItem | null = null;
  showAddForm = false;
  
  newItem: ReturnPolicyItem = {
    paragraph1: '',
    paragraph2: '',
    paragraph3: ''
  };
  ...
  ngOnInit() {
    this.loadReturnPolicyItems();
  }

  async loadReturnPolicyItems() {
    this.loading = true;
    try {
      this.returnPolicyItems = await this.supabaseService.getReturnPolicy();
    } catch (error) {
      console.error('Error loading return policy items:', error);
      alert('Error loading return policy items');
    } finally {
      this.loading = false;
    }
  }

  startEdit(item: ReturnPolicyItem) {
  ...
  async saveEdit() {
    ...
    try {
      ...
      console.error('Error updating return policy item:', error);
      alert('Error updating return policy item');
    } finally {
      this.loading = false;
    }
  }
  ...
  async deleteItem(id: number) {
    ...
    this.loading = true;
    try {
      await this.supabaseService.deleteReturnPolicy(id);
      await this.loadReturnPolicyItems();
      Swal.fire({
        title: 'تم الحذف!',
      ...
      console.error('Error deleting return policy item:', error);
      alert('Error deleting return policy item');
    } finally {
      this.loading = false;
    }
  }
  ...
  async addNewItem() {
    ...
    this.loading = true;
    try {
      await this.supabaseService.createReturnPolicy(this.newItem);
      await this.loadReturnPolicyItems();
      this.showAddForm = false;
      this.newItem = {
        paragraph1: '',
      ...
      console.error('Error adding return policy item:', error);
      alert('Error adding return policy item');
    } finally {
      this.loading = false;
    }
  }
